    ...
    <div>
      <nav className="w-full h-16 border border-b border-gray-800 flex justify-around items-center">
        ...
        <form className="px-5 flex items-center border border-gray-600 rounded-full">
          <input data-testid="search-movie" type="text" value={inputValue} onChange={handleChange} placeholder="Busque um filme" className="bg-transparent focus:outline-none p-2 text-white" />
          <BiSearchAlt2 />
        </form>
      </nav>
                      ...
                      <div className="mt-2">
                        {
                          moviesSearched && moviesSearched.results.length > 0 ? moviesSearched.results.slice(0, 15).map((movie: any) => {
                            return (
                              <Link href={`pages/movie/${movie.id}`} key={movie.id} data-testid="movie-searched">
